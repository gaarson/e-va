...
if start_file then
    ...
    local content = require("utils").read_file_range(config.PROJECT_ROOT .. "/" .. norm_start)
    if content then
...
while turn < MAX_TURNS do
    ...
    table.insert(messages, { role = "system", content = ctx:get_memory_block() })
    ...
    debug_dump = debug_dump .. "=== CHAT HISTORY ===\n"
    for i, m in ipairs(ctx.chat_history) do
